...
import { LiveFrame, liveFrame } from './live';
 ...
 *
 * **The last window of the vector is the one analysed**, which `liveFrame` does
...
function analyse(name: GoldenName): LiveFrame {
  return liveFrame(readGolden(name), GOLDEN_F0);
}

function content(frame: LiveFrame): Partial[] {
  return frame.partials.filter((partial) => partial.kind === 'partial');
}

function harmonics(frame: LiveFrame): number[] {
...
function lineAt(frame: LiveFrame, hz: number, toleranceHz = 2): Partial | undefined {
  return frame.partials.find((partial) => Math.abs(partial.hz - hz) <= toleranceHz);
}

describe('el comb del generador', () => {
  it('sale marcado como artefacto en los cuatro vectores', () => {
    for (const name of NAMES) {
      const frame = analyse(name);

      expect(frame.artefactHz, name).not.toBeNull();
      // The chip names a comb line, never a sideband: 2 756 Hz is the number
      // that tells the sample clock apart from a harmonic.
      expect((frame.artefactHz! / ARTEFACT_HZ) % 1, name).toBeCloseTo(0, 6);
    }
  });
...
describe('fmx-1op-sine — un operador, ratio 1.0', () => {
  const frame = analyse('fmx-1op-sine');
  ...
  it('da el suelo en dBFS absolutos, no relativo al pico', () => {
    ...
    expect(frame.floorDb).toBeGreaterThan(-133);
    expect(frame.floorDb).toBeLessThan(-128);
    expect(frame.floorDb).toBeLessThan(frame.peakDb);
  });
  ...
  it('vuelve a la cifra de la fase 0 cuando se le devuelve el pico', () => {
    ...
    const relativeToPeak = frame.floorDb - frame.peakDb;
  ...
  it('es una senoide: la fundamental y, muy abajo, el tercer armónico', () => {
    const loudest = frame.partials[0]!;
    ...
    const third = content(frame).find((partial) => partial.harmonic === 3);
    expect(third?.db).toBeCloseTo(-84, 0);

    const others = harmonics(frame).filter((harmonic) => harmonic !== 1);
  ...
  it('deja el comb como los dos picos más altos después de la fundamental', () => {
    ...
    const [, second, third] = frame.partials;
...
describe('fmx-ratio2-modlow — ratio 2:1, modulador bajo', () => {
  const frame = analyse('fmx-ratio2-modlow');

  it('sólo tiene armónicos impares', () => {
    const odd = harmonics(frame);
  ...
  it('reparte la energía como midió la fase 0: 1, 3, 5, 7 y nada entre medias', () => {
    expect(lineAt(frame, 261.93)?.db).toBeCloseTo(0, 0);
    expect(lineAt(frame, 785.24)?.db).toBeCloseTo(-29.5, 0);
    expect(lineAt(frame, 1308.7)?.db).toBeCloseTo(-68, 0);
    expect(lineAt(frame, 1832.1)?.db).toBeCloseTo(-82, 0);
...
describe('fmx-ratio2-modhigh — mismo ratio, modulador alto', () => {
  const frame = analyse('fmx-ratio2-modhigh');

  it('tiene el noveno armónico como pico, no la fundamental', () => {
    expect(frame.partials[0]!.harmonic).toBe(9);
...
describe('la trama', () => {
  it('sin nota no dibuja un eje que no tiene: ni curva ni barras', () => {
    ...
    const frame = liveFrame(readGolden('fmx-ratio2-modlow'), null);

    expect(frame.curve).toBeNull();
    expect(frame.harmonics).toBeNull();
    expect(frame.partials.length).toBeGreaterThan(0);
    expect(frame.artefactHz).not.toBeNull();
  });

  it('con nota trae la curva del eje 1×–32× y las dieciséis barras', () => {
    const frame = analyse('fmx-ratio2-modhigh');

    expect(frame.curve).toHaveLength(CURVE_POINTS);
    expect(frame.harmonics).toHaveLength(HARMONIC_BARS);
    ...
    expect(Math.max(...frame.curve!)).toBeCloseTo(0, 0);
    expect(Math.min(...frame.curve!)).toBeLessThan(-40);
  });

  it('no analiza lo que no tiene: menos de una ventana es la trama muerta', () => {
    const frame = liveFrame(new Float32Array(1024), GOLDEN_F0);

    expect(frame.curve).toBeNull();
    expect(frame.partials).toEqual([]);
  });
});
