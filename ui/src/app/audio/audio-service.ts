...
import { LiveTrama, NO_TRAMA, WATERFALL_FRAMES } from 'modx-dsp';
import { BACKEND_GATEWAY } from '../backend/backend-gateway';
import { equalTemperamentHz } from '../provenance/theory';
import { BridgeStats } from './bridge';
import { BlockMessage, FrameMessage, NoteMessage } from './audio.worker';
...
export interface AudioWorkerLike {
  postMessage(message: BlockMessage | NoteMessage, transfer: Transferable[]): void;
...
export class AudioService {
  ...
  start(): void {
    ...
    this.worker = worker;
    worker.addEventListener('message', ({ data }) => this.onFrame(data));
